...
An alternative philosophy inverts the problem: instead of trying to prove a piece of media is *fake*, prove that a piece of media is *real* by binding it, at creation time, to a trustworthy origin. This is the **provenance** paradigm now embodied by the C2PA Content Credentials standard and by secure-capture products such as Truepic and ProofMode [C2PA 2024; Truepic 2024; ProofMode 2023; Google 2025]. These systems are a major advance — C2PA supports hardware-backed signing, rich edit-history assertions, and ingredient hashes, and our goal is to **complement** rather than replace it. But the authenticity claim is carried, in the common case, in a **signed metadata container** attached to the file, and that container is a single point of failure: metadata can be deliberately stripped by platforms, lost in transcoding, or the binding itself can be attacked. In September 2025, a vulnerability in the Nikon Z6 III's C2PA implementation let a camera sign fabricated composites as authentic, forcing the vendor to suspend its authenticity service and revoke every certificate it had issued [Golaszewski et al. 2026]. If the credential is removed, the pixels carry no inherent evidence of their origin. PixelRoot targets exactly this gap, and can populate C2PA assertions while adding an in-pixel, ledger-witnessed binding that survives container loss.
...
**Design goals.** G1 Affirmative authenticity · G2 Hardware root · G3 Metadata-independence · G4 Public verifiability of the hard binding (the on-chain hash is verifiable by anyone with no secret) with delegated, key-holder verification of the in-pixel binding · G5 Minimal capture latency · G6 Imperceptibility · G7 Standardizability.
...
flowchart LR
  subgraph CAPTURE["On-device capture path"]
    ...
    S["CMOS sensor<br/>+ OTP ID"] --> M["Payload m<br/>pseudo-ID‖t‖(loc?)"]
...
`ID` is, **by default**, a per-device **pseudonymous** attestation identifier issued by the secure element (a rotating 48-bit token, not a global serial, unlinkable to the device without OEM cooperation), `t` a Unix timestamp from an attested RTC/NTP source, and `(φ, λ)` an **opt-in, off-by-default** quantized lat/long (~1e-4 degree resolution). The recommended default is pseudonymous ID with **no** location; when enabled, location may be coarsened or replaced by a salted commitment disclosed only under subpoena (§10).
...
**Proposition 2 (Carrier unpredictability).** Let the frame contain `N_B` blocks and let `m = n_c·R` carrier blocks each carry a secret antipodal chip. Without `k`, the probability of correctly recovering the carrier-and-chip assignment is `[ C(N_B, m) · 2^m ]⁻¹` — for 1920×1080 (`N_B = 32400` blocks), `m = 3200`, far below **10⁻³⁰⁰⁰**. Since `s = H(k‖m)` keys a cryptographic PRNG, distinct payloads induce computationally independent layouts, so observing many signatures leaks no usable information for a fresh `m`. The empirical counterpart — reading a marked image with 300 *wrong* keys — yields mean BER **0.503** and **0/300** correct recoveries (§9), matching the `p ≈ ½` random-guess model.
...
### 7.1 JPEG
JPEG's quantization matrix scales with quality factor `Q` and most aggressively discards high-frequency DCT energy [Wallace 1992]. This is exactly why the redesigned carrier (§5.4) lives in a *low-frequency* DCT band rather than at isolated (high-frequency) pixels: the chosen coefficient `(u,v) = (0,2)` sits in the lightly-quantized region of the luminance table and is preserved across a wide `Q` range, while spread-spectrum de-spreading over `R` blocks (eq. despread) suppresses quantization noise and host content by a further √R. Our real-libjpeg evaluation (§9, 24 Kodak images) confirms the analysis: the raw coded-bit BER stays low (≈1.6%) from Q=95 down to Q=10 and the Reed–Solomon code **recovers the exact payload on 23/24 images down to Q=20 (21/24 at the extreme Q=10)**, with the cliff appearing only at Q=5; the amplitude sweep in §9 locates the graceful decode-failure cliff, below which PixelRoot returns the safe **Unverified** rather than a false positive. The earlier fragile/semi-fragile tension is thus resolved by construction, not asserted.
...
We evaluate the redesigned spread-spectrum embedding (§5.4) on **real public images with real codecs**, following the WAVES (ICML 2024) watermark-robustness benchmarking practice. **Methodology.** The 24 Kodak natural photographs (768×512) carry a 128-bit payload protected by a real Reed–Solomon code RS(48,16) (`reedsolo`); the 384 coded bits are spread with repetition over 8×8 blocks at amplitude `A = 4` gray levels on the `(0,2)` carrier, embedded in the luma channel. Attacks use **actual libjpeg / WebP** (4:2:0 chroma subsampling), Gaussian noise and blur, bilinear resize round-trips, photometric edits, translational crop with grid resync, and rotation/resized-crop. Decoding uses the host-suppressed correlation de-spread (eq. despread) followed by RS decoding. The harness is released (`paper/experiments/`).

**Fidelity.** Embedding is imperceptible: mean **PSNR 38.2 dB, SSIM 0.942** (rising to ~41 dB at A=3). **Robustness** (Table below): on real content the raw channel is *not* error-free — mean coded-bit BER ≈ 1.6%, up to 13% on the most textured frame — but the RS code recovers the exact 128-bit payload on **23/24** images through real JPEG Q95→Q20 (21/24 at Q10), WebP, noise to σ=20, blur, resize 0.5–1.5×, all photometric edits, translational crops 1–3% with resync, and combined JPEG+resize. Failures are informative: one very-high-frequency frame exceeds the ECC capacity, and **geometric rotation (±1–2°) and resized-crop** drive BER to ≈0.5 — a documented limitation needing explicit rotation/scale sync. **Operating curve**: an amplitude sweep at JPEG Q40 over all 24 images exposes a genuine cliff — post-ECC decode rises from 0/24 at A=0.5 to 23/24 at A=4 (38 dB). **Security / false-accept**: reading marked images with 50 wrong keys (1200 trials) gives mean BER 0.4996 and 0/1200 recoveries; the correct-key decoder against the 24 *unmarked* images gives 0/24 false accepts; a **copy/transplant attack** is exposed (the forgery decodes to the *original* identity in 15/17 cases and passes as a *new* identity in 0/17). These match the `p ≈ ½` bound of Props. 2–3, whose closed-form RS false-accept probability is ≤ 1.8×10⁻⁶⁵.
...
| Gaussian noise σ≤20 | 0.017 | 23/24 |
| Gaussian blur σ=0.5 | 0.018 | 23/24 |
| Resize 0.75×, 1.5× | 0.025 | 22/24 |
| Bright/contrast/gamma ±10% | 0.017 | 21–23/24 |
| Crop 1%, 3% + resync | 0.018 | 23/24 |
| JPEG Q70 + resize 0.75× | 0.026 | 22/24 |
| JPEG Q5 (beyond range) | 0.221 | 0/24 |
| Rotation ±1–2° *[geom. limit]* | 0.50 | 0/24 |
| Resized-crop *[geom. limit]* | 0.49 | 0/24 |
| *Wrong-key read* (1200 trials) | 0.4996 | 0/1200 |
| *Forgery / unmarked* (24 trials) | 0.494 | 0/24 |
| *Copy attack* → new identity | — | 0/17 |

**Amplitude operating curve at JPEG Q40** (all 24 images) — imperceptibility vs. post-ECC robustness, showing the graceful failure cliff:
...
**Optional per-pixel localization layer.** When the sensor supports per-pixel gain (§5.1.1), PixelRoot adds a second, keyed high-resolution pattern serving as a spatial integrity map. On the 24 real Kodak images we embed a keyed ±δ (δ=4 gray levels) per-pixel pattern, create **48 real splices** (rectangular and elliptical foreign regions copied from other photographs, with pixel-level ground-truth masks), and detect tampering with a per-block matched-filter statistic (block 32 px). Reporting the metrics standard in forgery-localization (pixel-F1, p-F1, IoU, ROC-AUC), the layer reaches **ROC-AUC 0.974, pixel-F1 0.74, IoU 0.59, block TPR/FPR 0.914/0.037** on pristine media, degrading gracefully to AUC 0.954 under JPEG Q90. This layer carries no identity bits (it complements, not replaces, the robust payload channel) and is intended for lightly-processed forensic media.

| Localization metric | Pristine | JPEG Q90 |
|---|:--:|:--:|
...
**Passive PRNU baseline.** We implemented the standard passive sensor-fingerprint pipeline (denoising-residual fingerprint, NCC, PCE) on the same real content. Camera attribution is accurate on clean images, but the genuine–impostor NCC margin shrinks from +0.019 (clean) to +0.004 at JPEG Q50 and PCE drops ~18% — the well-known fragility of passive forensics under recompression that PixelRoot's *active*, error-corrected mark overcomes (it recovers the payload through Q10).

**Crypto micro-benchmarks (cross-validated).** SHA-256 over a 5.9 MB frame = 2.64 ms; seed + carrier selection = 0.43 ms; a Merkle tree over 10⁴ leaves builds in 5.8 ms with 14-deep (448-byte) proofs; analytic on-chain cost ≈ 45k gas per registration, amortizing to ≈ 4.5 gas/image at batch 10⁴.

**Scope and threats to validity.** These results validate the *algorithmic* core — carrier design, host-suppressed spread-spectrum detection, robustness to *real* JPEG/WebP/noise/blur/resize/photometric/crop attacks, false-accept behavior, and tamper localization — on *real public imagery* with *real codecs* and a *real Reed–Solomon* code, independent of silicon. They do not yet model demosaicing from the Bayer/RAW domain, real per-column gain calibration error, lens/optical effects, or the analog photo-electron domain; the PRNU comparison uses real Kodak *content* with the textbook multiplicative sensor model (per-camera RAW datasets are access-gated). The full harness, dataset script, figures, and the experiment plan (`paper/experiments/EXPERIMENT_PLAN.md`) are released for reproduction.

### 9.3 Remaining silicon- and codec-dependent validation
The following require hardware or a full pipeline and are future work: (E1) RAW/Bayer-domain capture and a multi-camera PRNU study on Dresden/VISION; (E2) video robustness vs. H.264/HEVC QP and GOP placement with keyframe-level Merkle commitments; (E3) an end-to-end re-capture and PRNU-copy [Goljan 2011] study; (E4) latency/gas on a public testnet (analytic estimate ≈ 45k gas/registration, ≈ 4.5 gas/image at batch 10⁴); and (E5) a silicon study of per-region/per-pixel gain modulation on a sensor evaluation board.
...
**Conflict of interest.** The author declares no conflict of interest regarding the publication of this article. The work received no external funding, and the author has no financial or personal relationships that could have inappropriately influenced it.

**Reproducibility & disclosure.** The reference implementation and the evaluation harness — the keyed spread-spectrum embed/extract of §5.4, the Reed–Solomon error-correction layer, the WAVES-aligned attack suite, and the tamper-localization and PRNU pipelines (in `paper/experiments/`) — are part of the open PixelRoot project. All measured numbers in §9.2 were produced on the public Kodak image set with production codecs (`libjpeg` JPEG and `libwebp` WebP) and are reproducible end-to-end from the released scripts and a pinned `requirements.txt`. On-chain notarization in the current prototype is *simulated* and labeled as such; the silicon-dependent per-pixel gain mechanisms are a feasibility analysis grounded in the cited fabrication literature, not fabricated results.

**Data availability.** The Kodak TrueColor image suite used in the experiments is publicly available. The provenance payloads are synthetic and contain no personal data. No human or animal subjects were involved in this work.
