...
**Fidelity.** Embedding is imperceptible: mean **PSNR 38.2 dB, SSIM 0.942** (rising to ~41 dB at A=3). **Robustness** (Table below): on real content the raw channel is *not* error-free — mean coded-bit BER ≈ 1.6%, up to 13% on the most textured frame — but the RS code recovers the exact 128-bit payload on **23/24** images through real JPEG Q95→Q10, WebP, noise to σ=20, blur, resize 0.5–1.5×, all photometric edits, translational crops 1–3% with resync, and combined JPEG+resize. Failures are informative: one very-high-frequency frame exceeds the ECC capacity, and **geometric rotation (±1–2°) and resized-crop** drive BER to ≈0.5 — a documented limitation needing explicit rotation/scale sync. **Operating curve**: an amplitude sweep at JPEG Q40 over all 24 images exposes a genuine cliff — post-ECC decode rises from 0/24 at A=0.5 to 23/24 at A=4 (38 dB). **Security / false-accept**: reading marked images with 50 wrong keys (1200 trials) gives mean BER 0.4996 and 0/1200 recoveries; the correct-key decoder against the 24 *unmarked* images gives 0/24 false accepts; a **copy/transplant attack** is exposed (the forgery decodes to the *original* identity in 15/17 cases and passes as a *new* identity in 0/17). These match the `p ≈ ½` bound of Props. 2–3, whose closed-form RS false-accept probability is ≤ 1.8×10⁻⁶⁵.
